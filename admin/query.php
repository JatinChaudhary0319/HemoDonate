      ...
      function clickme(){
        if(confirm("Do you really Want to Read ?")){
           ...
           document.getElementById("demo").innerHTML = "Read";
            <?php 
            echo '<div class="alert alert-info alert_dismissible">
            <b><button type="button" class="close" data-dismiss="alert">&times;</button></b><b>Pending Request "Read"</b></div>';

            $que_id = $_GET['id'];
            $sql1="update contact_query set query_status='1' where  id={$que_id}";
            ...
            while($row = mysqli_fetch_assoc($result)) { ?>
              <tr>
                  ...
                  <?php if($row['query_status']==1){  ?>
                    <td>Read<br></td>
                  <?php } else {?>
                    <td><a href="query.php?id=<?php echo $row['query_id'];?>" onclick="clickme()"><b id="demo">Pending</b></a><br></td>
                  <?php } ?>
